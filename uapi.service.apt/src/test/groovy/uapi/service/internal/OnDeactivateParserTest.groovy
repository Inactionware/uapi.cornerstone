...
/**
 * Unit test for OnDeactivateParser
 */
class OnDeactivateParserTest extends Specification {

    def 'Test create instance'() {
        when:
        def parser = new OnDeactivateParser()
    ...
    def 'Test parse'() {
        ...
        1 * classBuilder.findMethodBuilders(OnDeactivateParser.METHOD_ON_DEACTIVATE_NAME) >> []
        1 * classBuilder.addImplement(IServiceLifecycle.canonicalName) >> classBuilder
        1 * classBuilder.addMethodBuilder(_)
        1 * classBuilder.getTransience(OnDeactivateParser.MODEL_ON_DEACTIVATE) >> tmpModel
        def builderCtx = Mock(IBuilderContext) {
            getLogger() >> logger
        ...
        then:
        tmpModel.size() == 1
        List<String> methods = tmpModel.get(OnDeactivateParser.VAR_METHODS)
        methods.size() == 1
        methods.get(0) == 'test'
    }
    ...
    def 'Test add deactivate method if absent'() {
        given:
        def tmpModel = [:]
        def logger = Mock(LogSupport)
        def classElement = Mock(Element)
        ...
        when:
        parser.addOnDeactivateMethodIfAbsent(builderCtx, [element] as Set)
    ...
    def 'Test parse on existing element'() {
        ...
        def classBuilder = Mock(ClassMeta.Builder)
        1 * classBuilder.createTransienceIfAbsent(_, _) >> tmpModel
        1 * classBuilder.findMethodBuilders(OnDeactivateParser.METHOD_ON_DEACTIVATE_NAME) >> [Mock(MethodMeta.Builder) {
        ...
        0 * classBuilder.getTransience(OnDeactivateParser.MODEL_ON_DEACTIVATE) >> tmpModel
        def builderCtx = Mock(IBuilderContext) {
            getLogger() >> logger
            1 * findClassBuilder(classElement) >> classBuilder
